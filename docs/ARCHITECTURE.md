...
Steady-state (each goroutine, one file)
  goroutine ──► Process(ctx, file)
    ...
    Process ──► gpx.ParseFile(ctx): XML decode with cancellation-aware reads
...
On ctx cancel (SIGINT / SIGTERM / first file error)
  process(ctx, f) returns early at the next stage, parser read, decoded-point conversion, or segment boundary with a context error
